        ...
        >
            ...
            <ModalHeader onClose={onClose}>
                <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex items-center gap-2">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium ${quoteStatusColors[quote.status].bg
            ...
            <ModalBody>
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-800 dark:text-white font-medium">{quote.customerName}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{quote.customerEmail}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <p className="text-gray-800 dark:text-white">{formatDate(quote.createdDate)}</p>
                    </div>
                    <div>
                        <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
                            Valid Until
                        </h4>
                        <p className="text-gray-800 dark:text-white">{formatDate(quote.validUntil)}</p>
                    </div>
                </div>
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {quote.lineItems.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-800 dark:text-white/90 text-right font-medium">
                ...
                <div className="flex justify-end mb-6">
                    <div className="w-64 space-y-2">
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="text-gray-800 dark:text-white/90">{formatCurrency(quote.subtotal)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span className="text-gray-500 dark:text-gray-400">Tax ({quote.taxRate}%)</span>
                            <span className="text-gray-800 dark:text-white/90">{formatCurrency(quote.taxAmount)}</span>
                        </div>
                        <div className="flex justify-between text-base font-semibold pt-2 border-t border-gray-200 dark:border-gray-700">
                            <span className="text-gray-800 dark:text-white">Total</span>
                            <span className="text-brand-500">{formatCurrency(quote.total)}</span>
                        </div>
                    </div>
                </div>
            ...
            <ModalFooter>
                ...
                {/* Only show Convert button for non-rejected quotes */}
                ...
                <Button variant="primary" onClick={() => onEdit(quote)}>
                    Edit Quote
                </Button>
            </ModalFooter>
        </Modal>
